...
def main(memo, rall, road_net, env, gui, volume, ratio, model,count,lane):
    ...
    if multi_process:
        ...
        for p in list_cur_p:
            p.join()
    # record result
    run_cnt = dic_exp_conf_extra["RUN_COUNTS"]
    path_to_record = dic_path_extra['PATH_TO_WORK_DIRECTORY']
    path_to_traffic_env = os.path.join(path_to_record, "traffic_env.conf")
    with open(path_to_traffic_env,'r') as traffic_env:
        ...
        for i in range(0, num_intersections):
            path_to_vehicle = os.path.join(path_to_record, "vehicle_inter_{}.csv".format(i))
    ...
    result_dic = {
        "AverageTravelTime": dur
    }
    path_to_result = os.path.join(path_to_record, "aResult.json")
